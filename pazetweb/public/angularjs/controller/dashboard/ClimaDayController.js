...
app.controller('ClimaDayController', ['$scope','$http','Estaciones', function($scope,$http,Estaciones) {

	$scope.listEstaciones = [];
	$scope.listSeleccionados = [];

	var map = L.map('mapa_day',{
    ...
    /* ---------------- RENDERIZACIÓN DE LAS ESTACIONES CON DATOS CLIMATICOS DIARIOS SOBRE EL MAPA -----------------*/
	controlLoader.show();
	Estaciones.all_estn_clima_day_geojson().then(function(data) {
    ...
    function filtro(data, radio, type){
    	...
    	Estaciones.filtro_estn_in_poligon_cday_json(type,data,radio).then(function(data) {
	...
	//FILTRO DE ESTACIONES POR DEPARTAMENTOS Y CLIMA DIARIO
	$scope.searchWithDepto = function(){
		...
		if( codane == null || codane == '' ) {
			...
			Estaciones.estn_clima_day_depto_json(codane).then(function(data) {
	...
	$scope.searchWithMun = function(){
		...
		if( munid == null || munid == '' ) {
			...
			Estaciones.estn_clima_day_mun_json(munid).then(function(data) {
	...
	$scope.searchWithCodigo = function(){
		...
		if( codigo == null || codigo == '' ) {
			...
			Estaciones.estn_codigo_clima_day_json(codigo).then(function(data) {
	...
	//FILTRO DE ESTACIONES POR NOMBRE Y CLIMA DARIO
	$scope.searchWithName = function(){
		...
		if( nombre == null || nombre == '' ) {
			...
			Estaciones.estn_name_clima_day_json(nombre).then(function(data) {
